...
const websiteURL = "http://127.0.0.1:5500/1004cw-final/index.html";
...
test("link - vehicle search", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Vehicle Search" }),
  ).toBeVisible();
});
...
test("there is a <ul> inside <header> for navigation links", async ({
  page,
}) => {
...
test("use css flex to place navigation links horizontally", async ({
  page,
}) => {
  await expect(page.locator("header").locator("ul")).toHaveCSS(
    "display",
    "flex",
  );

  await expect(page.getByRole("link", { name: "Vehicle search" })).toHaveCSS(
    "flex",
    "0 1 auto",
  );
});
...
test("header should have padding 10px, margin 10px, and border 1px solid black", async ({
  page,
}) => {
  const space = "10px";
...
test("add a vehicle", async ({ page }) => {
  ...
  await expect(page.locator("#message-owner")).toContainText(
    "Owner added successfully",
  );

  await page.getByRole("button", { name: "Add vehicle" }).click();
  await expect(page.locator("#message-vehicle")).toContainText(
...
// Mines
// ! Website homepage after each test
// ! RESERT DATABASE after each test

//all fields empty shows error
...
// check owner disabled when owner empty
test("check owner disabled when owner empty", async ({ page }) => {
  await page.getByRole("link", { name: "Add a vehicle" }).click();
  await expect(
    page.getByRole("button", { name: "Check owner" }),
  ).toBeDisabled();
});

//check owner enabled when owner has text
test("check owner enabled when owner has text", async ({ page }) => {
  await page.getByRole("link", { name: "Add a vehicle" }).click();
  await page.locator("#owner").fill("Rachel");
  await expect(page.getByRole("button", { name: "Check owner" })).toBeEnabled();
});

// no owner match shows new owner button
test("no owner match shows new owner button", async ({ page }) => {
  await page.getByRole("link", { name: "Add a vehicle" }).click();
  await page.locator("#owner").fill("zzznomatch");
  await page.getByRole("button", { name: "Check owner" }).click();
  await expect(page.getByRole("button", { name: "New owner" })).toBeVisible();
});

//missing fields shows error
test("empty fields shows error 2", async ({ page }) => {
  await page.getByRole("link", { name: "Add a vehicle" }).click();
  await page.locator("#owner").fill("zzznomatch");
  await page.getByRole("button", { name: "Check owner" }).click();
  await page.getByRole("button", { name: "New owner" }).click();
...
//duplicate owner shows error
//try not to kys challenge
test("duplicate owner shows error", async ({ page }) => {
  await page.getByRole("link", { name: "Add a vehicle" }).click();
  await page.locator("#owner").fill("Rachel");
  await page.getByRole("button", { name: "Check owner" }).click();
  await page.getByRole("button", { name: "New owner" }).click();
